...
var TouchSlide = function TouchSlide(a) {
    ...
    for (var i in par) {
      ...
      for (var r in retn) {
        if (par[i][0] == "#") _retn.push(document.getElementById(par[i].replace("#", "")));else if (par[i][0] == ".") {
          ...
          var tag = retn[r].getElementsByTagName(par[i]);for (var j = 0; j < tag.length; j++) {
            _retn.push(tag[j]);
          }
        }
      }
  ...
  var doSwitchLoad = function doSwitchLoad(moving) {
    ...
    if (effect == "leftLoop") {
      switch (curIndex) {
        case 0:
          changeImg(conBoxSize);break;
        case 1:
          changeImg(conBoxSize + 1);break;
        case conBoxSize:
          changeImg(0);break;
        case conBoxSize + 1:
          changeImg(1);break;
    ...
    for (var i = 0; i < navObjSize; i++) {
      (function () {
        ...
        navObj[j].addEventListener('click', function (e) {
          clearTimeout(timeout);clearTimeout(endTimeout);index = j;doPlay();
        });
      })();
    }
  }
  if (nextBtn) {
    nextBtn.addEventListener('click', function (e) {
      if (loop == true || index != navObjSize - 1) {
        clearTimeout(timeout);clearTimeout(endTimeout);index++;doPlay();
      }
    });
  }
  if (prevBtn) {
